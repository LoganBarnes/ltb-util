...
SourceLocation::SourceLocation(std::filesystem::path file, int line) : filename(std::move(file)), line_number(line) {}
...
[[maybe_unused]] auto build_expected_message(std::string const& file_prefix,
    ...
    auto prefix    = (file_prefix.empty() ? std::string() : file_prefix);
    auto this_file = std::filesystem::path("src") / "ltb" / "util" / "error.cpp";
    return "[" + prefix + this_file.string() + ":" + std::to_string(line_number) + "] " + error_message;
}

} // namespace

TEST_CASE("[ltb][util][error] check error helpers") {
...
TEST_CASE("[ltb][util][error] check warning helpers") {
